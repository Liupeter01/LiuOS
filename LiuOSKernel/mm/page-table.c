...
)
{
    ...
    UINT64 pa_addr = physical_addr & PAGE_MASK;                 //物理地址起始

    /*需要创建的内存区域可能很大，需要以PGDIR_SIZE步长进行遍历*/
    phys_addr_t va_start = virt & PAGE_MASK;
    phys_addr_t va_end = is_pgd_arrive_end(va_start,virt_end);     //获得当前PUD段的终止地址
    for( ; va_end != virt_end ; )
    {  
        ...
        va_start = va_end;
        va_end = is_pgd_arrive_end(va_start,virt_end);
...
{
    console_puts("IN alloc_pgtable_pud\n");
    page_global_directory L0 = *((volatile  page_global_directory *)pgd);//取出 PGD结构的数据
    ...
    *------------------------------------------------------------------------------*/
    ...
    phys_addr_t va_end = is_pud_arrive_end(va_start,va_pud_end);    //获得当前PUD段的终止地址

    for( ; va_end!=va_pud_end ; )
    { 
        ...
        va_start = va_end;
        va_end = is_pud_arrive_end(va_start,va_pud_end);
        pa_addr += (va_end - va_start);                                 //进行物理地址的偏移                     
    }
}
...
{
    console_puts("IN alloc_pgtable_pmd\n");
    ...
    console_puts("IN alloc_pgtable_210\n");
    /*获取最新PUD页表项中的地址数值*/
    ...
    *------------------------------------------------------------------------------*/
    ...
    phys_addr_t va_end = is_pmd_arrive_end(va_start,va_pmd_end);                //获得当前PUD段的终止地址
    for(; va_start!=va_pmd_end ; )
    {  
        phys_addr_t va_end = is_pmd_arrive_end(va_start,va_pmd_end);                //获得当前PUD段的终止地址
        ...
        va_start = va_end;
        va_end = is_pmd_arrive_end(va_start,va_pmd_end);
        pa_addr += (va_end - va_start);                                 //进行物理地址的偏移                     
    }
}
...
{
    console_puts("IN alloc_pgtable_pte\n");
    ...
    *------------------------------------------------------------------------------*/
    ...
    for(phys_addr_t va_start = va_pte_start ; va_start!=va_pte_end ; va_start=is_pte_arrive_end(va_start,va_pte_end))
    {  
        set_pte_pgtable(
