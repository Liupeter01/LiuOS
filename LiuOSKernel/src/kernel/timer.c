 ...
 */
volatile uchar *kernel_gic_base = NULL;

/**
 * \ingroup drivers
 * Initializes kernel GIC
 * @param :use PA/VA address to init base address
 * @see peripherals/timer.h
 */
void gic_init(uint64_t GIC_addr)
{
	kernel_gic_base = (uchar*)GIC_addr;					//init uart base address(PA or VA)
}
...
void timer_1_init(uint32_t msec)
{
	...
	cur_val_1 = mmio_read((uint64_t)&kernel_gic_base[TIMER_CLO]);
	/* Increase it by an interval */
	cur_val_1 += interval_1;
	/* Set this value to be compared for System Timer 1 Interrupt */
	mmio_write((uint64_t)&kernel_gic_base[TIMER_C1], cur_val_1);
}
...
void timer_3_init(uint32_t msec)
{
	...
	/* Read current counter value */
	cur_val_3 = mmio_read((uint64_t)&kernel_gic_base[TIMER_CLO]);
	/* Increase it by an interval */
	cur_val_3 += interval_3;
	/* Set this value to be compared for System Timer 3 Interrupt */
	mmio_write((uint64_t)&kernel_gic_base[TIMER_C3], cur_val_3);
...
void timer_1_stop()
{
	...
	mmio_write((uint64_t)&kernel_gic_base[TIMER_C1], 0);
}

void timer_3_stop()
{
	/* Set 0 to compare register, in order to stop timer interrupts */
	mmio_write((uint64_t)&kernel_gic_base[TIMER_C3], 0);
}
...
void handle_timer_1_irq()
{
	...
	uint32_t timer_cs_1 = mmio_read((uint64_t)&kernel_gic_base[TIMER_CS]);
	timer_cs_1 |= TIMER_CS_M1;
	mmio_write((uint64_t)&kernel_gic_base[TIMER_CS], timer_cs_1);
...
void handle_timer_3_irq()
{
	...
	/* Acknowledge the interrupt by writing 1 to the TIMER_CS register */
	uint32_t timer_cs_3 = mmio_read((uint64_t)&kernel_gic_base[TIMER_CS]);
	timer_cs_3 |= TIMER_CS_M3;
	mmio_write((uint64_t)&kernel_gic_base[TIMER_CS], timer_cs_3);
...
{
	uint32_t counter_hi = mmio_read((uint64_t)&kernel_gic_base[TIMER_CHI]);
	uint32_t counter_lo = mmio_read((uint64_t)&kernel_gic_base[TIMER_CLO]);

	/* Check if high value didn't change, after setting it */
	while (counter_hi != mmio_read((uint64_t)&kernel_gic_base[TIMER_CHI])) {
		counter_hi = mmio_read((uint64_t)&kernel_gic_base[TIMER_CHI]);
		counter_lo = mmio_read((uint64_t)&kernel_gic_base[TIMER_CLO]);
	}
