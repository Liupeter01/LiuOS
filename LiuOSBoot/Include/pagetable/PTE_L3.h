...
#define PTE_ADDR_LOW	(((1UL << (48 - PAGE_SHIFT_LIUOS)) - 1) << PAGE_SHIFT_LIUOS)
...
/*物理地址转化页面同时清除垃圾信息*/
#define __phys_to_pageN(paddr) ((unsigned long)((paddr) >> PAGE_SHIFT_LIUOS))

/*先清除PTE页表项的垃圾数据，并将PGD属性加入到页表中*/
#define __pte_add_attribute(paddr,attribute)   __pte( __phys_to_pte_val(( __phys_to_pageN(paddr) << PAGE_SHIFT_LIUOS ) | (attribute)))

/*PTE页表是否抵达最底端(PTE是否越界)*/    
#define is_pte_arrive_end(virt,end_virt) ({ \
	unsigned long __boundary = ((virt) + PAGE_SIZE_LIUOS) & PAGE_MASK_LIUOS;	\
